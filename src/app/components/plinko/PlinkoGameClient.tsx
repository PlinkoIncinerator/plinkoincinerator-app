...
export default function PlinkoGameClient({
  ...
  const setDebouncedConnectionStatus = useCallback(
  ...
  useEffect(() => {
    ...
    const connectToServer = async () => {
      try {
        ...
        setIsReconnecting(false);
        console.log(
          "Auto-reconnect disabled. Please use the Reconnect button if needed."
        );
      }
    };
  ...
  const handleDisconnect = (data: { reason?: string }) => {
    ...
    if (data.reason && !data.reason.includes("transport")) {
      ...
      console.log(
        "Temporarily disconnected from the Plinko server. Reconnecting..."
      );
    }
    ...
    setTimeout(() => {
      ...
      const reconnectDelay = Math.min(
        Math.pow(1.3, reconnectAttempts) * 1000,
        10000
      );
      ...
      reconnectTimeoutRef.current = setTimeout(() => {
        ...
        setReconnectAttempts((prev) => {
          ...
          console.log(
            `Reconnection attempt ${newValue} of ${maxReconnectAttempts}`
          );

          // Check if we've reached max attempts
          if (newValue >= maxReconnectAttempts) {
            console.error(
              "Unable to connect to the Plinko server after multiple attempts. Please refresh the page."
            );
            setIsReconnecting(false);
  ...
  const throttledPlay = useCallback(() => {
    ...
    try {
      ...
      console.log(
        `PlinkoGameClient: Sending play request (ID: ${playId})`,
        lastOptions
      );
      ...
      try {
        ...
        console.log(
          `PlinkoGameClient: Play request sent successfully (ID: ${playId})`
        );
      } catch (playError) {
        console.error(
          `PlinkoGameClient: Error in plinkoService.play (ID: ${playId})`,
          playError
        );

        // Try to reconnect and retry the play request
        console.log(
          `PlinkoGameClient: Attempting to reconnect and retry (ID: ${playId})`
        );
        plinkoService
          .reconnect()
          .then(() => {
            console.log(
              `PlinkoGameClient: Reconnected, retrying play (ID: ${playId})`
            );
            plinkoService.play(lastOptions);
          })
          .catch((reconnectError) => {
            console.error(
              `PlinkoGameClient: Reconnect failed (ID: ${playId})`,
              reconnectError
            );
            setError(
        ...
        try {
          ...
          plinkoService
            ...
            .catch((err) => {
              ...
              // Try to play anyway
              console.log(
                "Error refreshing game state, sending play request anyway..."
              );
              throttledPlay();
            });
        } catch (error) {
  ...
  const handleAnimationComplete = () => {
    console.log("Animation complete", latestResult);
  ...
  const handleTestModeToggle = () => {
    ...
    console.log(
      "PlinkoGameClient: Test mode toggle from",
      isTestMode,
      "to",
      newTestMode
    );
    ...
    const immediateControls = {
      ...
      currentBalance: newTestMode
        ? initialBalance || 10
        : currentDisplayBalance || gameState.balance || 0,
    ...
    if (onRegisterControls) {
      console.log(
        "PlinkoGameClient: Forcing immediate update of controls for test mode change:",
        newTestMode
      );
    ...
    setCurrentDisplayBalance(
      newTestMode ? initialBalance || 10 : gameState.balance || 0
    );
    ...
    >
      ...
      {showPopupResult && latestResult && (
        <div className="fixed top-1/4 inset-x-0 z-50 flex justify-center pointer-events-none">
          <div
            className={`relative bg-gray-900 bg-opacity-90 border-2 ${
              latestResult.finalMultiplier > 1
      ...
      <div
        className={`flex flex-col ${
          isMobileView ? "w-full h-full" : "md:flex-row"
        } gap-6`}
      >
            ...
            <div className="absolute top-2 left-2 z-30">
              <button
                ...
                className={`flex items-center px-2 py-1 rounded-full ${
                  isTestMode ? "bg-purple-600" : "bg-gray-700"
                } text-xs transition-colors duration-200`}
              >
                <span
                  className={`inline-block h-3 w-3 rounded-full mr-1.5 ${
                    isTestMode ? "bg-white" : "bg-gray-500"
                  }`}
                ></span>
                Test Mode
              </button>
              ...
              {gameState.gameHistory
                ...
                .map((game, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`bg-gray-900 p-2 rounded-lg flex justify-between items-center ${
                        isWin
        ...
        <div className="mt-16 bg-gray-800 rounded-lg p-4 text-white overflow-hidden">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="text-sm text-gray-400">
              Showing{" "}
              {Math.min((currentPage - 1) * itemsPerPage + 1, totalGames)} -{" "}
              {Math.min(currentPage * itemsPerPage, totalGames)} of {totalGames}
            </div>
          </div>
                ...
                {paginatedHistory.map((game, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {isWin ? "+" : ""}
                        {formatAmount(profit).replace(" SOL", "")}
                      </td>
                      <td className="py-2 px-4">
                        <span
